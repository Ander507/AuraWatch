...
function mapRoomErr(msg: string): never {
...
export const POST: RequestHandler = async ({ params, request, cookies }) => {
	...
	try {
		...
		mapRoomErr(String(e?.message || e));
	}
};

/** DELETE /api/rooms/[slug] — host deletes their room */
export const DELETE: RequestHandler = async ({ params, locals }) => {
	if (!isTursoConfigured()) throw error(503, 'Group rooms need Turso configured');

	const session = await locals.auth();
	const userId = session?.user?.id;
	if (!userId) throw error(401, 'Sign in to delete a room');

	const slug = normalizeRoomSlug(params.slug || '');
	if (!slug) throw error(404, 'Room not found');

	try {
		await deleteOwnedRoom({ slug, userId });
		return json({ ok: true, slug });
	} catch (e: any) {
		const msg = String(e?.message || e);
		if (msg === 'room_not_found') throw error(404, 'Room not found');
		if (msg === 'forbidden') throw error(403, 'Only the host can delete this room');
		if (msg === 'auth_required') throw error(401, 'Sign in to delete a room');
		if (msg === 'turso_missing') throw error(503, 'Group rooms need Turso configured');
		console.error('delete room failed', msg);
		throw error(500, 'could not delete room');
	}
};
